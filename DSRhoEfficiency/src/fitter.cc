...
Fitter::Fitter(const char* evtgen_filepath, const char* gsim_filepath, const char* output_dir) {
	...
	if (strstr(evtgen_filepath, ".root")) {
		...
		evtgen_dataset_ = RooDataSet::read(evtgen_filepath, RooArgList(thetat_, thetab_, phit_, dt_, dec_type_));
	}
...
void Fitter::PlotVar(RooRealVar& var) {
	...
	canvas_var_ = new TCanvas(TString(var.GetName()) + "_canvas", TString(var.GetTitle()) + " canvas", 500, 500);
...
void Fitter::PlotVars2D(RooRealVar& var1, RooRealVar& var2) {
	if (canvas_var_) delete canvas_var_;
	canvas_var_ = new TCanvas(TString(var1.GetName()) + "_" + TString(var2.GetName()) + "_evtgen_canvas",
			TString(var1.GetTitle()) + "_" + TString(var2.GetName()) + " evtgen canvas", 500, 500);

	TH2D* evtgen_histo = static_cast<TH2D*>(evtgen_dataset_->createHistogram("evtgen_histo", var1, RooFit::YVar(var2)));
	...
	TH2D* gsim_histo = static_cast<TH2D*>(gsim_dataset_->createHistogram("gsim_histo", var1, RooFit::YVar(var2)));
...
void Fitter::PlotEfficiency(RooRealVar& var, bool plot_model, bool legend_position_top, bool legend_position_left) {
	if (canvas_eff_) delete canvas_eff_;
	canvas_eff_ = new TCanvas(TString(var.GetName()) + "_eff_canvas", TString(var.GetTitle()) + " eff canvas", 500, 500);
	...
	if (plot_model) {
		...
		plot_eff_pull_->GetXaxis()->SetTickLength(0.03 * pad_eff->GetAbsHNDC() / pad_pull->GetAbsHNDC());
		plot_eff_pull_->GetXaxis()->SetTitle(TString(var.GetTitle()));
		plot_eff_pull_->GetXaxis()->SetTitleOffset(4.0);
		plot_eff_pull_->GetXaxis()->SetLabelOffset(0.01 * pad_eff->GetAbsHNDC() / pad_pull->GetAbsHNDC());
...
void Fitter::PlotEfficiency2D(RooRealVar& var1, RooRealVar& var2) {
	if (canvas_eff_) delete canvas_eff_;
	canvas_eff_ = new TCanvas(TString(var1.GetName()) + "_" + TString(var2.GetName()) + "_eff_canvas",
	...
	printf("Correlation %s : %s is %f\n", var1.GetName(), var2.GetName(), eff_histo->GetCorrelationFactor());
	...
	canvas_eff_->SetName(TString(var1.GetName()) + "_" + TString(var2.GetName()) + "_pdf_eff_canvas");
	canvas_eff_->SetTitle(TString(var1.GetTitle()) + "_" + TString(var2.GetName()) + " PDF eff canvas");
	...
	canvas_eff_->SetName(TString(var1.GetName()) + "_" + TString(var2.GetName()) + "_eff_pull_canvas");
	canvas_eff_->SetTitle(TString(var1.GetTitle()) + "_" + TString(var2.GetName()) + " eff pull canvas");
