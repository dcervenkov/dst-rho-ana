...
// Constructor
//
panther2TTreeModule::panther2TTreeModule() : Module()
{
...
// Destructor
//
panther2TTreeModule::~panther2TTreeModule()
{
}
...
//
void panther2TTreeModule::hist_def(void)
{
   BelleTupleManager* tm = BASF_Histogram;
   //ROOT stuff
   std::string SRFile = outFileName;
   SRFile.append(".root");
   //char RFile[100] = SRFile.c_str();
   rootFile = new TFile(SRFile.c_str(), "RECREATE");
   rootFile->cd("");

   //h1 = new TH1F("h1","my histogram",100,1,100);
   rootTree = new TTree("Tree", "My TTree");
   //rootTree->Branch("particles","MyParticle", particles);
   rootTree->Branch("numParticles",&r_numParticles,"numParticles/I");
   rootTree->Branch("id",r_id,"id[numParticles]/I");
   rootTree->Branch("idhep",r_idhep,"idhep[numParticles]/I");
   rootTree->Branch("mother",r_mother,"mother[numParticles]/I");
   rootTree->Branch("da1",r_da1,"da1[numParticles]/I");
   rootTree->Branch("da2",r_da2,"da2[numParticles]/I");
   rootTree->Branch("p",r_p,"p[numParticles][5]/D");
   rootTree->Branch("v",r_v,"v[numParticles][4]/D");
...
// Initialize
//
void panther2TTreeModule::init(int* status)
{
   std::cout << "\n*** panther2TTree Initialization ***\n" << std::endl;
...
// Finish
//
void panther2TTreeModule::term( void )
{
   //std::cout << "beg term" << std::endl;
   //outFile.close();
   //rootTree->Scan("id:idhep:mother:da1:da2"); 
   rootFile->cd("");
   rootFile->Write();
   rootFile->Close();
   std::cout << "rootFile closed" << std::endl;
...
// Start run
//
void panther2TTreeModule::begin_run(BelleEvent *, int *)
{
...
// Process event
//
void panther2TTreeModule::event(BelleEvent* evtptr, int* status)
{
   ...
   //if ((runNo==iRun) && (iEvt>=evtNo) && (iEvt<(evtNo+nEvts))) {
      ...
      //std::cout << "Relevant Event" << std::endl;
      // Call HEPEVT manager
	 ...
	 //std::cout << "Adding particle " << numParticles << std::endl;
      ...
      else std::cout << "Not calling print" << std::endl;
   //}
}
...
// Print the tree
//
void panther2TTreeModule::print(HepAList<Gen_hepevt> list, int iExp, int iRun, int iEvt)
{
   ...
   // Particles
   for(num = 0; num <= list.index(list.last()); num++) {
      Gen_hepevt& event = *list[num];

      r_id[num] = num+1;
      r_idhep[num] = event.idhep();
      r_mother[num] = event.moFirst();
      r_da1[num] = event.daFirst();
      r_da2[num] = event.daLast();
      for(int j = 0; j < 5; j++)
	 r_p[num][j] = event.P(j);
      for(int j = 0; j < 4; j++)
	 r_v[num][j] = event.V(j);

      //std::cout << "r_idhep = " << r_idhep[num] << std::endl;
   } 
   r_numParticles = num;
   //std::cout << "r_numParticles = " << r_numParticles << std::endl;
   rootTree->Fill();
   //std::cout << "Filling tree" << std::endl;
   //rootTree->Scan("id:idhep");
   //rootTree->Print();
}

// Return realName
std::string panther2TTreeModule::getRealName(int idhep) {
