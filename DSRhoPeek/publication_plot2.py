#!/usr/bin/env python
"""This script generates publication quality plots from a ROOT file created
by DSRhoPeek"""
...
def create_histogram_list():
    """Create a list of histograms to be plotted

    The syntax is:
        file_name, expression, cuts, x_axis_label, log_scale, x_axis_min, x_axis_max
    """
    histograms = []
...
def create_data_list():
    """Create a list of datafiles to be processed"""
    data = []
...
def make_publication_plots():
    ...
    for histogram_name, histogram_formula, histogram_cuts, axis_title, logaritmic, minimum, maximum in histograms:
        for file_name, hist_num, channel in data:
            file = ROOT.TFile(file_name)
            tree = file.Get("h2000")
            canvas.SetLogy(logaritmic)

            if minimum != "":
                if histogram_cuts != "":
                    histogram_cuts += "&&"
                histogram_cuts += histogram_formula + ">" + str(minimum)
            if maximum != "":
                if histogram_cuts != "":
                    histogram_cuts += "&&"
                histogram_cuts += histogram_formula + "<" + str(maximum)
            ...
            if not tree.Draw(histogram_formula, histogram_cuts, to_same):
                print("WARNING: Nothing to print for {} with {} cuts!".format(
                    histogram_formula, histogram_cuts))
                continue
            ...
            canvas.SetTicky()
            canvas.SaveAs(
                os.path.join(plot_dir, histogram_name + "_" + channel + extra_text + image_format))
