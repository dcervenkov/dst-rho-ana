...
int main(int argc, char const *argv[]) {
    ...
    printf("Processing %s...\n", dataname);
    AnalyzeDataset(dataname, -1, 20, 50, 5, 1, 10, 10, "fast");
    return 0;
}
...
Double_t Ylm(UInt_t l, Int_t m, Double_t cosTheta1, Double_t phi) {
    ...
    if (m > 0)
        return TMath::Power(-1, m) * TMath::Sqrt2() * ROOT::Math::sph_legendre(l, m, theta1) *
               TMath::Cos(m * phi);  // integral is 1
    else if (m < 0)
        return TMath::Power(-1, m) * TMath::Sqrt2() * ROOT::Math::sph_legendre(l, -m, theta1) *
               TMath::Sin(-m * phi);  // integral is 1
    else
        return ROOT::Math::sph_legendre(l, 0, theta1);  // integral is 1
}
...
void AnalyzeDataset(TString dataset, Int_t max_entries, Int_t nBins3D, Int_t nBins1D,
    ...
    if (dataset == "BsJpsiPhi") {
        ...
        t_data->Add(
            "/home/reznicek/data/BsJpsiPhi/MC11/Acceptance/mu4mu4MoreStatsforAccpbb/*.root");
        angle_names[0] = "CDFPhi";
        angle_title[0] = "#phi";  // phi must be 1st !!!, cos theta1/2 to follow
        angle_names[1] = "CDFCostheta";
        angle_title[1] = "cos #theta";
        angle_names[2] = "CDFCosPsi";
        angle_title[2] = "cos #psi";
        // nBins3D = 20;
        // nBins1D = 100;
        ...
        t_data->Add(
            "/home/reznicek/data/SemileptonicRareB/Data2012/MCntuples/cut/"
            "skim_208446_sel_red_cut.root");
        angle_names[0] = "Extra_helicityAnglePhi";
        angle_title[0] = "#phi";  // phi must be 1st !!!, cos theta1/2 to follow
        angle_names[1] = "Extra_helicityCosThetaL";
        angle_title[1] = "cos #theta_{L}";
        angle_names[2] = "Extra_helicityCosThetaK";
        angle_title[2] = "cos #theta_{K}";
        ...
        angle_names[1] = "costht";
        angle_title[1] = "cos #theta_{t}";
        angle_names[2] = "costhb";
        angle_title[2] = "cos #theta_{b}";

    } else if (dataset == "DC_gsim_trans") {
        selection = "";
        t_data = new TChain("h2000");
        t_data->Add("../data/DSRho-mdst_basf2_mod_real_unmod_all.root");
        angle_names[0] = "phit";
        angle_title[0] = "#phi_{t}";  // phi must be 1st !!!, cos theta1/2 to follow
        angle_names[1] = "transtht";
        angle_title[1] = "#theta_{t}";
        angle_names[2] = "transthb";
    ...
    } else if (dataset == "DC_evtgen_trans") {
        selection = "";
        t_data = new TChain("dataset");
        t_data->Add("from_ascii.root");
        angle_names[0] = "phit";
    ...
    TF3 *f3 =
        new TF3("f_SumAklmYlmPk", SumAklmYlmPk, -TMath::Pi(), TMath::Pi(), -1, 1, -1, 1, nPar + 3);
    f3->SetParameter(0, 1);
    ...
    TH3D *h3 =
        new TH3D("h3", "h3", nBins3D, -TMath::Pi(), TMath::Pi(), nBins3D, -1, 1, nBins3D, -1, 1);
    ...
    for (Int_t iter = 0; iter < f3->GetNpar() - 3; iter++) {
        ...
        condition =  // chi2["prob_phi"]                 > 0.05 &&
            ...
            chi2["chi2NDF_phi_cosTheta1"] < 2 && chi2["chi2NDF_phi_cosTheta2"] < 2 &&
    ...
    if (nBins1D != nBins3D) {
        TH3D *h3_fine = new TH3D("h3_fine", "h3_fine", nBins1D, -TMath::Pi(), TMath::Pi(), nBins1D,
                                 -1, 1, nBins1D, -1, 1);
        t_data->Draw(angle_names[2] + ":" + angle_names[1] + ":" + angle_names[0] + ">>+ " +
