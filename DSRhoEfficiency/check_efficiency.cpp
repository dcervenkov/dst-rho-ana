...
int check_efficiency() {
    gStyle->SetPalette(kViridis);
    gStyle->SetOptStat(0);

    TFile* gsim_file = new TFile("DC_gsim_mod.root", "read");
    TH3F* gsim_histo = (TH3F*)((TH3F*)gsim_file->Get("R__TF3"))->Clone("gsim_histo");
    gsim_histo->Rebin3D();
    TFile* evtgen_file = new TFile("DC_evtgen.root", "read");
    ...
    for (int x = 0; x < eff_histo->GetNbinsX(); x++) {
        for (int y = 0; y < eff_histo->GetNbinsY(); y++) {
            for (int z = 0; z < eff_histo->GetNbinsZ(); z++) {
                ...
                if (eff_histo->GetBinContent(bin) > 1 || eff_histo->GetBinContent(bin) < 0) {
                    ...
                    if (eff_histo->GetBinContent(bin) < 0) {
                        printf("WTF???\n");
                    }
                }
            }
