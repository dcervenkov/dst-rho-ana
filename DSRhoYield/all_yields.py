...
def recover_yield(filename):
	file = TFile(filename)
	sig_plus_cf = float(str(file.Get("all;1").GetLineWith("n_signal_plus_cross_model")).split()[1])
	sig_plus_cf_error = float(str(file.Get("all;1").GetLineWith("n_signal_plus_cross_model")).split()[3])
	f_sig_cf = float(str(file.Get("signal_plus_crossfeed;1").GetLineWith("signal_plus_cross_f")).split()[1])
...
def recover_count(dir):
	...
	count = chain.Draw("de", "(evmcflag==1||evmcflag==7||evmcflag==8)&&de>-0.14", "goff")
	return count
...
for channel in channels:
	...
	for stream in streams:
		sig_yield, sig_yield_error = recover_yield("plots/" + channel + "/stream" + str(stream) + "/fit_results.root")
