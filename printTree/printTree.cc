...
// Constructor
//
printTreeModule::printTreeModule() : Module()
{
  strcpy(outFileName, "mdstTree.txt");
  sprintf(strVars, "XYZE");
...
// Destructor
//
printTreeModule::~printTreeModule()
{
}

//
// Define ntuples & histograms
//
void printTreeModule::hist_def(void)
{
...
// Initialize
//
void printTreeModule::init(int* status)
{
  std::cout << "\n*** printTree Initialization ***\n" << std::endl;
  ...
  if (qqOnly)
    std::cout << " Will print QQ particles only" << std::endl;
  else
    std::cout << " Will print all particles    " << std::endl;

  std::cout   << " Variables to print are:     " << strVars << std::endl;

  if (whichB==0) {
    std::cout << " Print both B mezons         " << std::endl;
  } else {
    std::cout << " Print B #"                    << whichB << std::endl;
  }

  std::cout   << " Will show decay depth of:   " << decDepth << std::endl;

  std::cout   << "\n Printing following ... "    << std::endl;
  std::cout   << "  Run: "                       << runNo << std::endl;
  std::cout   << "  Events: "                    << evtNo << " - " << evtNo+nEvts << std::endl;
...
// Finish
//
void printTreeModule::term( void )
{
  outFile.close();
}
...
// Start run
//
void printTreeModule::begin_run(BelleEvent *, int *)
{
    //DC: getEventInfo doesn't work for non-gsim data - returns only 0's; this is a workaround
    //m_iEvt = 0;
}
...
// Process event
//
void printTreeModule::event(BelleEvent* evtptr, int* status)
{
  // Get experiment -> run -> event number
  int iExp    = 0;
  int iRun    = 0;
  int iEvt    = 0;
  bool mcFlag = false;

  getEventInfo(iExp, iRun, iEvt, mcFlag);

  //DC: getEventInfo doesn't work for non-gsim data - returns only 0's; this is a workaround
  //iEvt = m_iEvt++;

  if ((runNo==iRun) && (iEvt>=evtNo) && (iEvt<(evtNo+nEvts))) {
     ...
     for (std::vector<Gen_hepevt>::iterator it = EvtMgr.begin(); it != EvtMgr.end(); it++) {

        Gen_hepevt& event = *it;
        PartList.append(event);
     }

     if (PartList.last()) print(PartList, iExp, iRun, iEvt);
  }
}
...
// Print the tree
//
void printTreeModule::print(HepAList<Gen_hepevt> list, int iExp, int iRun, int iEvt)
{
  ...
  if (index(strVars,ID)) { outFile << "|" << std::setw(12) << "    ID"   ; varlen++; }
  if (index(strVars,PX)) { outFile << "|" << std::setw(12) << "    PX"   ; varlen++; }
  if (index(strVars,PY)) { outFile << "|" << std::setw(12) << "    PY"   ; varlen++; }
  if (index(strVars,PZ)) { outFile << "|" << std::setw(12) << "    PZ"   ; varlen++; }
  if (index(strVars,VX)) { outFile << "|" << std::setw(12) << "    VX"   ; varlen++; }
  if (index(strVars,VY)) { outFile << "|" << std::setw(12) << "    VY"   ; varlen++; }
  if (index(strVars,VZ)) { outFile << "|" << std::setw(12) << "    VZ"   ; varlen++; }
  if (index(strVars,E))  { outFile << "|" << std::setw(12) << "     E"   ; varlen++; }
  if (index(strVars,M))  { outFile << "|" << std::setw(12) << "     M"   ; varlen++; }
  if (index(strVars,ST)) { outFile << "|" << std::setw(12) << "   ISTHEP"; varlen++; }
  if (index(strVars,PI)) { outFile << "|" << std::setw(12) << "     PID" ; varlen++; }
  outFile << "|" << std::endl;
  ...
  while (converge<list.index(list.last())) {
     ...
     if ((level<decDepth)&&(dooutput)) {
        ...
        if (index(strVars,ID)) outFile << std::setw(10) << i+1            <<  std::setw(3) << "";
        if (index(strVars,PX)) outFile << std::setw(10) << event.PX()     <<  std::setw(3) << "";
        if (index(strVars,PY)) outFile << std::setw(10) << event.PY()     <<  std::setw(3) << "";
        if (index(strVars,PZ)) outFile << std::setw(10) << event.PZ()     <<  std::setw(3) << "";
        if (index(strVars,VX)) outFile << std::setw(10) << event.VX()     <<  std::setw(3) << "";
        if (index(strVars,VY)) outFile << std::setw(10) << event.VY()     <<  std::setw(3) << "";
        if (index(strVars,VZ)) outFile << std::setw(10) << event.VZ()     <<  std::setw(3) << "";
        if (index(strVars,E))  outFile << std::setw(10) << event.E()      <<  std::setw(3) << "";
        if (index(strVars,M))  outFile << std::setw(10) << event.M()      <<  std::setw(3) << "";
        if (index(strVars,ST)) outFile << std::setw(10) << event.isthep() <<  std::setw(3) << "";
        if (index(strVars,PI)) outFile << std::setw(10) << event.idhep()  <<  std::setw(3) << "";
        outFile << std::endl;
     }
     ...
     if (event.daFirst() && (!qqOnly || list[event.daFirst()-1]->isthep()>0)) {
        ...
        if (event.moFirst()) {
           ...
           if (list[event.moFirst()-1]->daLast()-1 != i) leaders+=filled;
           else                                          leaders+=blank;
        }
     ...
     // Has mother
        ...
        // Not last daughter
        if (list[event.moFirst()-1]->daLast()-1 != i) i++;
        ...
        // Last daughter: find next mother
           ...
           while (list[list[i]->moFirst()-1]->daLast()-1==i) {
              ...
              if (!i) return; // It happens... (when there's an orphan in the event)
           }
     ...
     else return; // No mother, no daughter! Poor girl...

     // Prepare for next particle (modified by jwicht)
     if ((level<decDepth)&&(dooutput)) outFile  << std::setw(0) << leaders << "+-> ";
...
// Return realName
std::string printTreeModule::getRealName(int idhep) {
