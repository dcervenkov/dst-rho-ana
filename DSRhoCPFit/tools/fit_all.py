...
DEFAULT_OPTIONS = ["--MC=1", "--cpus=1", "--log"]
COMMAND = "./DSRhoCPFit"
...
CHANNELS_COMBINATIONS += [
    ",".join([c1, c2]) for c1 in CHANNELS for c2 in CHANNELS if c1 != c2
]
...
def decode_arguments():
    ...
    parser.add_argument(
        ...
        dest="channels",
        choices=CHANNELS_COMBINATIONS,
    )
    parser.add_argument(
        "-d", "--td", action="store_true", help="do time-dependent fits"
    )
    parser.add_argument(
        "-i", "--ti", action="store_true", help="do time-independent fits"
    )
    parser.add_argument(
        "-m",
        "--component",
        dest="components",
        choices=["CR", "CRSCF", "all"],
        action="append",
        help="do CR-only fits",
    )
    parser.add_argument(
        "-f", "--force", action="store_true", help="force rewrite of existing results"
    )
    parser.add_argument(
        "-j", "--cpus", help="number of cores to use", default=1, type=int
    )
    parser.add_argument(
        "-s", "--streams", default=6, type=int, help="force rewrite of existing results"
    )
        ...
        for channel in options.channels:
            for component in options.components:
                output_dir = os.path.join(
                    RESULTS_DIR,
                    "_".join([channel.replace(",", "-"), time_type, component]),
                )
                ...
                for stream in range(0, options.streams):
                    ...
                    cmd.append("--config=" + CONFIG_BASENAME + str(stream) + ".json")
                    excluded_channels = set(CHANNELS) - set(channel.split(","))
                    if excluded_channels:
                        cmd.append("--exclude-channels=" + ",".join(excluded_channels))
                    output_file = os.path.join(output_dir, "stream" + str(stream))
...
def main():
    ...
    commands, dirs_to_create, dirs_to_delete, dirs_to_skip = create_fit_cmds(options)
    ...
    if user_input == "" or user_input.lower() == "y":
        ...
        subprocess.run(
            "parallel --will-cite --bar --nice 10 -j "
            + str(options.cpus)
            + " < "
            + TEMP_FILE,
            cwd="..",
