...
//	fit_result_ = active_model_->fitTo(*data_set_, RooFit::Save(), RooFit::Minimizer("Minuit2"), RooFit::Hesse(1), RooFit::Minos(1), RooFit::NumCPU(4));
	fit_result_ = active_model_->fitTo(*data_set_, RooFit::Save(), RooFit::Minimizer("Minuit2"), RooFit::NumCPU(4));
}
...
void Fitter::WriteFitResults() {
	...
	macro.AddLine(line);
	snprintf(line, 1000, "%i entries, fit status: %i", data_set_->numEntries(), fit_result_->status());
	macro.AddLine(line);
...
/**
 * Plot the data, full model PDFs (or a component thereof if Setup was called) as well as signal and background PDFs.
 */
void Fitter::Plot() {
	RooPlot* frame_de = de_.frame();
	...
	frame_pull->addPlotable(hpull_clone, "P"); // We plot again without bars, so the points are not half covered by bars

	TLegend* legend = 0;
	// The plotting of the components has to take place after creating the pull histogram, otherwise we
	// would have to make sure the correct (full) pdf is used for the computation
	switch (active_component_) {
	case Components::signal:
		active_model_->plotOn(frame_de, RooFit::Components(sig_cb_), RooFit::LineColor(29), RooFit::LineStyle(kDashed));
		active_model_->plotOn(frame_de, RooFit::Components(sig_gaus_), RooFit::LineColor(32), RooFit::LineStyle(kDashed));
		break;

	case Components::crossfeed:
		active_model_->plotOn(frame_de, RooFit::Components(cross_exponential_),	RooFit::LineColor(40), RooFit::LineStyle(kDashed));
		active_model_->plotOn(frame_de, RooFit::Components(cross_gaus_), RooFit::LineColor(49), RooFit::LineStyle(kDashed));
		break;

	case Components::signal_plus_crossfeed:
		active_model_->plotOn(frame_de, RooFit::Components(signal_model_), RooFit::LineColor(4), RooFit::LineStyle(kDashed));
		active_model_->plotOn(frame_de, RooFit::Components(cross_model_), RooFit::LineColor(7), RooFit::LineStyle(kDashed));
		break;
	...
	case Components::all:
		active_model_->plotOn(frame_de, RooFit::Name("signal"), RooFit::Components(signal_model_), RooFit::LineColor(4), RooFit::LineStyle(kDashed));
		active_model_->plotOn(frame_de, RooFit::Name("crossfeed"), RooFit::Components(cross_model_), RooFit::LineColor(7), RooFit::LineStyle(kDashed));
		active_model_->plotOn(frame_de, RooFit::Name("background"), RooFit::Components(bkg_model_), RooFit::LineColor(5), RooFit::LineStyle(kDashed));
...
void Fitter::SPlotFull(TChain* chain) {
	...
	RooDataSet* dataw_bg  = new RooDataSet(data_set_->GetName(),data_set_->GetTitle(),data_set_,*data_set_->get(),0,"n_bkg_sw");
	...
	data_set_->plotOn(frame_sig_thetab, RooFit::MarkerColor(kRed), RooFit::Cut("(evmcflag==1||evmcflag==7||evmcflag==8)||(candsel!=0&&(evmcflag!=1&&evmcflag!=7))"));

	RooPlot* frame_bkg_thetab = thetab_.frame(100);
	dataw_bg->plotOn(frame_bkg_thetab, RooFit::DataError(RooAbsData::SumW2));
	data_set_->plotOn(frame_bkg_thetab, RooFit::MarkerColor(kRed), RooFit::Cut("!((evmcflag==1||evmcflag==7||evmcflag==8)||(candsel!=0&&(evmcflag!=1&&evmcflag!=7)))"));
...
void Fitter::SPlotSC(TChain* chain) {
	...
	RooAddPdf sc_model_ ( "sc_model","", RooArgList(signal_model_, cross_model_), RooArgList(n_signal_model_, n_cross_model_));

	RooStats::SPlot* sDataX = new RooStats::SPlot("sData","An SPlot", *data_set_, &sc_model_, RooArgList(n_signal_model_, n_cross_model_) );

	RooDataSet* dataw_sig = new RooDataSet(data_set_->GetName(),data_set_->GetTitle(),data_set_,*data_set_->get(),0,"n_signal_model_sw");
	RooDataSet* dataw_bg  = new RooDataSet(data_set_->GetName(),data_set_->GetTitle(),data_set_,*data_set_->get(),0,"n_cross_model_sw");

	TCanvas* can = new TCanvas("splot_sc", "splot_sc", 500, 1000);

	RooPlot* frame_sig_thetab = thetab_.frame(100);
	dataw_sig->plotOn(frame_sig_thetab, RooFit::DataError(RooAbsData::SumW2));
	data_set_->plotOn(frame_sig_thetab, RooFit::MarkerColor(kRed), RooFit::Cut("(evmcflag==1||evmcflag==7||evmcflag==8)"));

	RooPlot* frame_bkg_thetab = thetab_.frame(100);
	dataw_bg->plotOn(frame_bkg_thetab, RooFit::DataError(RooAbsData::SumW2));
	data_set_->plotOn(frame_bkg_thetab, RooFit::MarkerColor(kRed), RooFit::Cut("(candsel!=0&&(evmcflag!=1&&evmcflag!=7))"));
...
void Fitter::SPlotSB(TChain* chain) {
	...
	RooAddPdf sb_model_ ( "sb_model","", RooArgList(signal_model_, bkg_model_), RooArgList(n_signal_model_, n_bkg_model_));

	RooStats::SPlot* sDataX = new RooStats::SPlot("sData","An SPlot", *data_set_, &sb_model_, RooArgList(n_signal_model_, n_bkg_model_) );

	RooDataSet* dataw_sig = new RooDataSet(data_set_->GetName(),data_set_->GetTitle(),data_set_,*data_set_->get(),0,"n_signal_model_sw");
	RooDataSet* dataw_bg  = new RooDataSet(data_set_->GetName(),data_set_->GetTitle(),data_set_,*data_set_->get(),0,"n_bkg_model_sw");
...
void Fitter::SPlotCB(TChain* chain) {
	...
	RooRealVar n_cross_model_ ("n_cross_model", "n_{cf}", 20000, 1000, 100000);
	RooRealVar n_bkg_model_ ("n_bkg_model", "n_{bkg}", 20000, 1000, 100000);
	RooAddPdf cb_model_ ( "cb_model","", RooArgList(cross_model_, bkg_model_), RooArgList(n_cross_model_, n_bkg_model_));

	RooStats::SPlot* sDataX = new RooStats::SPlot("sData","An SPlot", *data_set_, &cb_model_, RooArgList(n_cross_model_, n_bkg_model_) );

	RooDataSet* dataw_sig = new RooDataSet(data_set_->GetName(),data_set_->GetTitle(),data_set_,*data_set_->get(),0,"n_cross_model_sw");
	RooDataSet* dataw_bg  = new RooDataSet(data_set_->GetName(),data_set_->GetTitle(),data_set_,*data_set_->get(),0,"n_bkg_model_sw");
	...
	RooPlot* frame_sig_thetab = thetab_.frame(100);
	dataw_sig->plotOn(frame_sig_thetab, RooFit::DataError(RooAbsData::SumW2));
	data_set_->plotOn(frame_sig_thetab, RooFit::MarkerColor(kRed), RooFit::Cut("(candsel!=0&&(evmcflag!=1&&evmcflag!=7))"));
...
double Fitter::GetCorrelation(TChain* chain, const RooRealVar& var1, const RooRealVar& var2, bool save_plot) {
	if (data_set_) delete data_set_;
	data_set_ = new RooDataSet("data_set", "data_set", chain, RooArgSet(de_, evmcflag_, candsel_, thetab_), data_cut_);
	data_set_->Print();
